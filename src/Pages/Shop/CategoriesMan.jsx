...
const CategoriesMan = () => {
  const state = useSelector((state) => state.menSectionSlice);
  const {t} = useTranslation();
  return (
    <div className="h-fit w-full p-4">
      <div className="w-full h-full">
        <div className={`my-4 ${i18n.dir() === "rtl" ? "border-r-4" : "border-l-4"} border-pink-500 rounded px-2`}>
          <h3 className="text-xl font-semibold text-indigo-600">{t("CategoriesForMen")}</h3>
        </div>
            ...
            state?.map((item) => (
              ...
              >
                ...
                <p className="pt-2  flex justify-between items-center ">
                  <span className="inline-block text-gray-700  ">{item.title}</span>
